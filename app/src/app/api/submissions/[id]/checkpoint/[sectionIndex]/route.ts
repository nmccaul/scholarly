...
export async function POST(
  ...
  if (!Number.isFinite(sectionIndex) || sectionIndex < 0) {
    return apiError('Invalid sectionIndex', 400)
  }
  ...
  await saveInitialTextResponse(id as SubmissionId, sectionIndex, body.text.trim())

  const conversation = [{ role: 'student' as const, text: body.text.trim() }]
  ...
  if (maxFollowUps > 0 && evaluation.followUpQuestion) {
  ...
  // No follow-ups configured — force-unlock immediately
